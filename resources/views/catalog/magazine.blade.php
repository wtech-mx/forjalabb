...
<div class="magazine-experience" data-magazine>
    ...
    <div class="magazine-stage" data-magazine-stage>
        <div class="magazine-book">
            ...
            @php $bundleImages = collect([$bundle->image_url])->merge($bundle->photos->map->image_url)->filter()->unique()->values(); @endphp
            <article class="magazine-page magazine-product-page bundle-edition" data-magazine-page>
                <div class="magazine-product-photo" data-magazine-gallery><div class="magazine-photo-frame">@if($bundleImages->isNotEmpty())<img src="{{ $bundleImages->first() }}" alt="{{ $bundle->name }}" loading="lazy" data-magazine-gallery-main>@else<div class="magazine-photo-placeholder"><i class="bi bi-box-seam-fill"></i></div>@endif<span>Paquete ForjaLab</span></div>@if($bundleImages->count() > 1)<div class="magazine-gallery-strip" aria-label="Galería de {{ $bundle->name }}"><small><i class="bi bi-images"></i> Galería <b>{{ $bundleImages->count() }} fotos</b></small><div>@foreach($bundleImages->take(6) as $index => $image)<button class="{{ $index === 0 ? 'active' : '' }}" type="button" data-magazine-gallery-thumb="{{ $image }}" aria-label="Abrir foto {{ $index + 1 }}"><img src="{{ $image }}" alt="" loading="lazy"></button>@endforeach</div></div>@endif</div>
            ...
            @php $productImages = collect([$product->image_url])->merge($product->photos->map->image_url)->filter()->unique()->values(); @endphp
            <article class="magazine-page magazine-product-page {{ $loop->iteration % 2 === 0 ? 'reverse' : '' }}" data-magazine-page>
                <div class="magazine-product-photo" data-magazine-gallery><div class="magazine-photo-frame">@if($productImages->isNotEmpty())<img src="{{ $productImages->first() }}" alt="{{ $product->name }}" loading="lazy" data-magazine-gallery-main>@else<div class="magazine-photo-placeholder"><i class="bi bi-stars"></i></div>@endif<span>{{ $product->badge ?: 'Personalizable' }}</span></div>@if($productImages->count() > 1)<div class="magazine-gallery-strip" aria-label="Galería de {{ $product->name }}"><small><i class="bi bi-images"></i> Galería <b>{{ $productImages->count() }} fotos</b></small><div>@foreach($productImages->take(6) as $index => $image)<button class="{{ $index === 0 ? 'active' : '' }}" type="button" data-magazine-gallery-thumb="{{ $image }}" aria-label="Abrir foto {{ $index + 1 }}"><img src="{{ $image }}" alt="" loading="lazy"></button>@endforeach</div></div>@endif</div>
